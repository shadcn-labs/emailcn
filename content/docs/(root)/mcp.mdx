...
Install items with **`npx shadcn@latest add @emailcn/<base>/<name>`** or the **`https://emailcn.run/r/<base>/<name>.json`** URL on each docs page (bases: `react-email`, `mjml-react`, `jsx-email`), then import from **`@/components/email/...`** as usual. See the [registry docs](/docs/registry) for configuring the `@emailcn` namespace.
