...
Install items with **`npx shadcn@latest add @emailcn/<name>`** or the **`https://emailcn.vercel.app/r/<style>/<name>.json`** URL on each docs page (styles: `react-email`, `mjml-react`, `jsx-email`), then import from **`@/components/emails/...`** as usual. See the [registry docs](/docs/registry) for configuring the `@emailcn` namespace per style.
